...
function HomeView({go,notify}:{go:(t:Tab)=>void;notify:(a:string,b:string)=>void}) {
  return <section className="screen home-screen">
    <div className="welcome"><div><p>WELCOME TO MENSION</p><h1>営業の出会いを、<br/><em>ここから資産に。</em></h1></div><div className="rank"><small>THIS MONTH</small><strong>0</strong><span>CONTACTS</span></div></div>
    <article className="hero-card"><div className="hero-grid"/><div className="hero-copy"><span className="live"><i/>MENSION AI ACTIVE</span><h2>その一枚を、<br/>成果に変える。</h2><p>名刺を撮れば、顧客登録もファーストコンタクトも。<br/>商談につながる次の一手をAIが支えます。</p></div><button onClick={()=>go('scan')}><span>名刺をスキャン</span><b>→</b></button><div className="monogram">@</div></article>
    <div className="summary-head"><div><small>TODAY&apos;S PERFORMANCE</small><h3>本日のパフォーマンス</h3></div><span>まだデータはありません</span></div>
