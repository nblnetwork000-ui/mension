...
type AiContextConfig={company:boolean;role:boolean;website:boolean;profile:boolean};
...
export default function Home() {
  ...
  const [showAiRange,setShowAiRange]=useState(false);
  const [aiContext,setAiContext]=useState<AiContextConfig>({company:true,role:true,website:true,profile:true});
  ...
  useEffect(() => {
    ...
    const loadData=async()=>{
      ...
      const savedAiContext=userData.user.user_metadata?.mension_ai_context as Partial<AiContextConfig>|undefined;if(savedAiContext)setAiContext(current=>({...current,...savedAiContext}));
  ...
  const saveAiContext=async(next:AiContextConfig)=>{if(supabase){const {error}=await supabase.auth.updateUser({data:{mension_ai_context:next}});if(error){notify('設定を保存できませんでした','もう一度お試しください');return;}}setAiContext(next);setShowAiRange(false);notify('反映範囲を保存しました','次回のメール文面生成から反映されます');};
  ...
  const sendBulkMail=async()=>{
    ...
    const replace=(value:string,customer:Customer)=>value.replaceAll('{{会社名}}',customer.company).replaceAll('{{氏名}}',customer.name).replaceAll('{{役職}}',customer.role).replaceAll('{{送信者名}}',sender.displayName).replaceAll('{{AI生成文}}',buildAiLine(customer,aiContext,profile));
  ...
  return <main className={`app-shell ${loading?'is-loading':'is-ready'}`}>
    ...
    {composeCustomer&&<MailComposer customer={composeCustomer} template={mailTemplate} sender={sender} aiContext={aiContext} profile={profile} connected={!!googleAccessToken} onConnect={connectGoogle} onClose={()=>setComposeCustomer(null)} onSend={sendMail}/>} 
    ...
    {showAiRange&&<AiRangeEditor value={aiContext} onClose={()=>setShowAiRange(false)} onSave={saveAiContext}/>} 
...
function SettingsView({sender,googleConnected,sendMode,setSendMode,autoGreeting,setAutoGreeting,signature,setSignature,companyContext,setCompanyContext,notify,onOpenGuide,onOpenTemplate,onOpenProfile,onOpenSender,onOpenAiRange}:{sender:SenderConfig;googleConnected:boolean;sendMode:string;setSendMode:(v:string)=>void;autoGreeting:boolean;setAutoGreeting:(v:boolean)=>void;signature:boolean;setSignature:(v:boolean)=>void;companyContext:boolean;setCompanyContext:(v:boolean)=>void;notify:(a:string,b:string)=>void;onOpenGuide:()=>void;onOpenTemplate:()=>void;onOpenProfile:()=>void;onOpenSender:()=>void;onOpenAiRange:()=>void}) { return <section className="screen settings"><PageHead kicker="PREFERENCES" title="設定" sub="あなたらしいフォローを自動化"/>
...
function buildAiLine(customer:Customer,config:AiContextConfig,profile:UserProfile){const parts=[config.company&&customer.company?`${customer.company}でのお取り組み`:config.company?'貴社でのお取り組み':'今後のお取り組み',config.role&&customer.role?`${customer.role}としてのお考え`:'',config.website&&customer.website?'Webサイトで拝見した事業内容':'',config.profile&&profile.company?`${profile.company}との連携の可能性`:''].filter(Boolean);return `${parts.join('や')}について、ぜひ改めてお話を伺えれば幸いです。`;}

function AiRangeEditor({value,onClose,onSave}:{value:AiContextConfig;onClose:()=>void;onSave:(value:AiContextConfig)=>void}){const [draft,setDraft]=useState(value);const rows:Array<[keyof AiContextConfig,string,string]>=[['company','会社名・事業内容','顧客の会社情報を文面へ反映'],['role','役職・肩書','相手の立場に合わせて表現を調整'],['website','Webサイト情報','登録された企業サイトの情報を反映'],['profile','自社プロフィール','自社の事業内容との接点を反映']];return <div className="template-overlay" role="dialog" aria-modal="true"><div className="template-card ai-range-card"><div className="template-head"><div><small>AI CONTEXT</small><h2>企業情報の反映範囲</h2></div><button onClick={onClose}>×</button></div><p>メール文面を生成するときに使用する情報を選択してください。</p><div className="context-options">{rows.map(([key,title,detail])=><button key={key} onClick={()=>setDraft({...draft,[key]:!draft[key]})}><i className={draft[key]?'on':''}>{draft[key]?'✓':''}</i><span><strong>{title}</strong><small>{detail}</small></span></button>)}</div><div className="sender-security"><span>♢</span><small>選択した情報は文面生成にのみ使用し、メール本文へ自動で全情報を列挙しません。</small></div><div className="template-actions"><button onClick={onClose}>キャンセル</button><button onClick={()=>onSave(draft)}>設定を保存</button></div></div></div>;}

function MailComposer({customer,template,sender,aiContext,profile,connected,onConnect,onClose,onSend}:{customer:Customer;template:MailTemplate;sender:SenderConfig;aiContext:AiContextConfig;profile:UserProfile;connected:boolean;onConnect:()=>void;onClose:()=>void;onSend:(subject:string,body:string)=>Promise<void>}){
  const replace=(value:string)=>value.replaceAll('{{会社名}}',customer.company).replaceAll('{{氏名}}',customer.name).replaceAll('{{役職}}',customer.role).replaceAll('{{送信者名}}',sender.displayName).replaceAll('{{AI生成文}}',buildAiLine(customer,aiContext,profile));
